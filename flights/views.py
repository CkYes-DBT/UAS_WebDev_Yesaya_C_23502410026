...
def search_flight(request):
    if request.method == "POST":
        ...
        origin, origin_known = get_iata_code(origin_input)
        destination, destination_known = get_iata_code(destination_input)
        ...
        # Check if airports are unknown
        if not origin:
            error = f"Bandara '{origin_input}' tidak ditemukan dalam database kami. Silakan gunakan kode IATA 3 huruf yang valid (contoh: CGK untuk Jakarta, DPS untuk Bali)."
        elif not destination:
            error = f"Bandara '{destination_input}' tidak ditemukan dalam database kami. Silakan gunakan kode IATA 3 huruf yang valid (contoh: BKK untuk Bangkok, SIN untuk Singapore)."
        else:
            ...
            # Add warning if code is unknown (3 letters but not in database)
            if not origin_known and len(origin) == 3:
                warning = f"Peringatan: Kode '{origin}' tidak dikenali dalam database kami. Kami akan mencoba mencari penerbangan, tapi mungkin tidak ditemukan."
            elif not destination_known and len(destination) == 3:
                warning = f"Peringatan: Kode '{destination}' tidak dikenali dalam database kami. Kami akan mencoba mencari penerbangan, tapi mungkin tidak ditemukan."
                    ...
                    try:
                        ...
                        if "400" in error_msg:
                            if "INVALID DATE" in error_msg.upper() or "past" in error_msg.lower():
                                ...
                                error = f"Kode airport '{destination}' tidak valid atau tidak dikenali oleh Amadeus API. Pastikan menggunakan kode IATA yang benar (contoh: BKK untuk Bangkok, DPS untuk Bali)."
                            else:
        ...
        context = {
            "origin": origin or origin_input,
            "destination": destination or destination_input,
            "origin_input": origin_input,
            "destination_input": destination_input,
            "conversion_note": conversion_note if 'conversion_note' in locals() else None,
            "warning": warning,
